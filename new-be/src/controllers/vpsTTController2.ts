...
function shouldAutoGenerateDokuOnInvoice(): boolean {
...
export const generateInvoiceAndMarkProcess = async (req: Request, res: Response) => {
  ...
  try {
    ...
    let sharedDokuPayment: NonNullable<ITTVpsDetail['doku_payment']> | undefined;
    if (autoGenerateDoku) {
    ...
    await session.withTransaction(async () => {
      for (const doc of sortedDocs) {
        ...
        if (sharedDokuPayment) {
          doc.doku_payment = sharedDokuPayment;
        } else {
          doc.doku_payment = undefined;
        }
        doc.status = 'PROCESS';
