...
const shouldAutoGenerateDokuOnInvoice = (): boolean => {
  const raw = String(process.env.DOKU_AUTO_GENERATE_ON_INVOICE ?? 'true').trim().toLowerCase();
  return !['false', '0', 'off', 'no', 'disabled'].includes(raw);
};

const resolveDokuCustomer = async (
  detail: any,
  fallback?: { phone?: string; address?: string }
) => {
  let subscriber: any = null;
  if (detail.subscriber_id) {
    subscriber = await Subscriber.findById(detail.subscriber_id).lean();
  }
  if (!subscriber) {
    subscriber = await Subscriber.findOne({
      ...
      delete_date: null,
    }).sort({ update_date: -1 }).lean();
  }
  if (!subscriber) {
    subscriber = await Subscriber.findOne({ toko: detail.toko, delete_date: null }).sort({ update_date: -1 }).lean();
  }

  return normalizeDokuCustomer({
    id: detail.kode_subscriber || subscriber?.kode,
    name: detail.toko,
    phone: subscriber?.nomor_telepon
      || subscriber?.no_hp_pic
      || subscriber?.no_hp_owner
      || fallback?.phone
      || detail.invoice_meta?.customer?.phone,
    address: subscriber?.alamat
      || fallback?.address
      || detail.invoice_meta?.customer?.address,
    city: subscriber?.daerah || detail.daerah,
    country: 'ID',
  });
};
...
export const generateInvoiceAndMarkProcess = async (req: Request, res: Response) => {
  try {
    ...
    const autoGenerateDoku = shouldAutoGenerateDokuOnInvoice();
    let sharedDokuPayment: any | undefined;
    if (autoGenerateDoku) {
      const customer = invoice.customer || {};
      const dokuCustomer = await resolveDokuCustomer(docs[0], {
    ...
    for (const doc of docs) {
      ...
      doc.doku_payment = sharedDokuPayment || undefined;
      doc.status = 'PROCESS';
      doc.update_date = now;
    ...
    res.json({
      ...
      invoice,
      doku_auto_generate_enabled: autoGenerateDoku,
      doku_payment: sharedDokuPayment || null,
    });
  } catch (error: any) {
    if (error instanceof DokuApiError) {
      ...
      return res.status(502).json({
        message: error.message,
        doku_status: error.status,
        doku_request_id: error.requestId,
        doku_details: error.details,
      });
    }
    console.error('Error in generateInvoiceAndMarkProcess:', error);
    res.status(500).json({ message: error?.message || 'Server error', error });
  }
};
